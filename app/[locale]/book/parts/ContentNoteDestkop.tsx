...
const NoteCol: React.FC<NoteColProps> = ({
	...
	return (
		<div
			className={`flex flex-col ${isOpen ? '' : 'h-44'} border border-gray-200 mt-2 px-4 pt-4 pb-2 rounded-lg ${left ? 'lg:rounded-r-3xl' : 'lg:rounded-l-3xl'}`}
		>
			<span>
				{title}
				{'...'}
			...
			<div
				className={`my-2 h-0.5 rounded-full ${color ? color : 'bg-PrimaryColor'}`}
			></div>
