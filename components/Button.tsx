...
export const IconTextBotton: React.FC<IconTextSmBtnProps> = ({
  ...
  return (
    <button className="hidden md:block" onClick={onClick}>
      <div
        className={`mx-3 flex px-3 py-2 rounded-2xl ${isActive ? "bg-PrimaryColor dark:bg-PrimaryColor-bgDark86" : "bg-PrimaryColor-12 dark:bg-PrimaryColor-bgDark10"}`}>
        <Image className="mr-3" src={isActive ? icon : activeIcon} alt="" />
        <span
          className={`${isActive ? "text-white dark:text-black-background" : "text-black-200  dark:text-white-darkText01"}`}>
          {text}
        </span>
      </div>
    </button>
